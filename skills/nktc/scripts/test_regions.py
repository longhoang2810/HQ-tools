...
class RegionConfigTest(unittest.TestCase):
    def test_config_has_34_merged_province_sheet_names(self):
        ...
        self.assertIn("Hồ Chí Minh", names)
        self.assertIn("Hải Phòng", names)
        self.assertIn("Cà Mau", names)
    ...
    def test_default_config_has_hcm_and_creates_sheet(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Hồ Chí Minh", result.sheetnames)
            self.assertEqual(result["Hồ Chí Minh"]["B5"].value, "DN HCM")
