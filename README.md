...
## Cách dễ nhất: mở `index.html` bằng trình duyệt

Không cần cài Python, không cần terminal. Double-click file
[`index.html`](index.html) (hoặc mở bằng trình duyệt bất kỳ, kể cả không
...
Tiện ích trên trang: nút **Dán ví dụ** / **Xóa**, phím tắt **Ctrl+Enter**,
tự tra ngay khi dán, chip thống kê số chất cần Giấy phép, và nút
**In / Lưu PDF** (tự mở hết chi tiết trước khi in). Ghi kèm `% hàm lượng`
...
lại `python3 build_html.py` để cập nhật `index.html` — file này sinh cả
...
vì text, thêm bước OCR trước khi đưa vào `scan.py`/`index.html`.
