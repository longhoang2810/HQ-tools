...
function hintHtml(cas) {
  ...
  for (const e of PL3_EXCLUDED.filter(x => x.cas === cas)) {
    ...
    // Summary la NHAN thuan (so muc), khong dien giai lai noi dung phap ly ->
    // khong the lech voi PL3_EXCLUDED_NOTE ben core.py.
    out += foldNote("excl-note", `ⓘ Ghi chú "Ngoại trừ" — mục ${esc(stt)}`,
      `<div class="fold-body">${esc(PL3_EXCLUDED_NOTE.replace("{stt}", stt))}</div>`);
  }
