...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Ghim ma CAS lam o Ket luan chui XUONG DUOI cot ghim khi cuon het co, va pill
     bi cat mat may chu DAU. Verdict phu dinh chi khac verdict khang dinh o dung
     chu dau -> cat dau la doc luot ra NGUOC nghia. Chan bang be ngang: bop bang
     lai de (cot ghim + cot Ket luan) <= be ngang khung, luc do cuon het co thi
     Ket luan van nam tron ben phai cot ghim. Do o 320/375/414px: du 20px dem. */
  @media (max-width: 700px) {
    table { min-width: 400px; }
    td.mota { min-width: 110px; }
    /* min-width:0 -> cot Ket luan co lai thay vi nuot phan du cua bang; padding
       trai rong lam vung dem, lech vai px thi an vao dem chu khong cat chu. */
    td:last-child { min-width: 0; padding-left: 20px; }
  }
  ...
  per.forEach((p, i) => {
    ...
    const sub = matches.length
      ? matches.map(c => {
          const s = casStatus(c);
          return {
            match: byName ? esc((rowsFor(c)[0] || {}).name_vn || c) : esc(c),
            concl: `<span class="pill ${s.badge}">${esc(s.text)}</span>${hintHtml(c)}`,
          };
        })
