...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Muc chu "\\203A" xoay 90 do khi mo — cung ngon ngu voi details.fold o duoi
     trang, de cho nao thu gon duoc cung nhin ra ngay. */
  details.note-fold > summary { cursor: pointer; list-style: none; display: flex; gap: 7px; align-items: baseline; }
  details.note-fold > summary::-webkit-details-marker { display: none; }
  details.note-fold > summary::before { content: "\\203A"; font-size: 1.25em; line-height: 1; transition: transform .15s; }
  details.note-fold[open] > summary::before { transform: rotate(90deg); }
  .fold-body { margin-top: 7px; }
...
// Ghi chu dai 200-300 chu; tren dien thoai o Ket luan chi ~150px nen no thanh
// khoi cao ~370px, keo ca dong hang dai ngoang. Thu gon bang <details> native:
// man rong mo san, man hep dong san. Tin hieu CO ghi chu gi phai nam trong
// <summary> — chi giau phan dien giai, khong bao gio giau canh bao.
function foldNote(cls, summary, body) {
  // typeof: test chay ban JS nay bang node, o do khong co matchMedia -> mo san.
  const narrow = typeof matchMedia === "function" && matchMedia("(max-width: 700px)").matches;
  const open = narrow ? "" : " open";
  return `<details class="${cls} note-fold"${open}><summary>${summary}</summary>${body}</details>`;
}
...
function hintHtml(cas) {
  ...
  for (const e of PL3_EXCLUDED.filter(x => x.cas === cas)) {
    const stt = e.ngoai_tru_khoi.replace(/\\.$/, "");
    // Summary phai TU NOI DUOC dieu quan trong (ma nay CO in trong bang PL III
    // nhung nghi dinh LOAI TRU no), khong duoc la cai nhan tron kieu "Ghi chu
    // Ngoai tru — muc 26": man hep gap san, doc cai nhan thi khong biet gi ca.
    // Van KHONG go tay chu nao — cat ngay tu PL3_EXCLUDED_NOTE cua core.py o cau
    // dau, nen hai noi khong the lech nhau (ly do cua ban cu van duoc giu).
    const note = PL3_EXCLUDED_NOTE.replace("{stt}", stt);
    const cut = note.indexOf(". ");
    const head = cut < 0 ? note : note.slice(0, cut + 1);
    const rest = cut < 0 ? "" : note.slice(cut + 2);
    out += foldNote("excl-note", esc(head),
      rest ? `<div class="fold-body">${esc(rest)}</div>` : "");
  }
  ...
  if (hints.length) {
    ...
    out += foldNote("fam-hint", `<b>${esc(PL3_HINT_PREFIX)}</b>`, `<ul>${items}</ul>`);
  }
  return out;
}
...
function lineNameHint(entry) {
  ...
  // Canh bao ("co ten nhung khong co ma") o summary, huong dan xu ly o body.
  return foldNote("fam-hint", `⚠ Mô tả có nhắc tên hóa chất trong dữ liệu nhưng KHÔNG kèm mã CAS: ${esc(names.join(", "))}`,
    `<div class="fold-body">Kết luận trên chỉ tính theo mã CAS — chuyển sang <b>Tìm theo tên chất</b> để tra riêng.</div>`);
}
...
function lineCasHint(entry) {
  ...
  return foldNote("fam-hint", `⚠ Dòng có mã CAS (${esc(entry.cas.join(", "))}) — chế độ tra theo tên bỏ qua`,
    `<div class="fold-body">Chuyển sang <b>Tìm theo mã CAS</b> để tra theo mã.</div>`);
}
...
// Ban in la thu ket vao ho so: ghi chu dang thu gon KHONG duoc mat khi in.
addEventListener("beforeprint", () => {
  document.querySelectorAll("details.note-fold").forEach(d => { d.open = true; });
});
